...
function RoleBasedRoutes({ allowedRoles, children }) {
    ...
    const hasRequiredRole = allowedRoles ?  Array.isArray(allowedRoles) ? allowedRoles.includes(user?.role) : user?.role === allowedRoles : true;
                ...
                <div className="card-body p-5">
                  ...
                  <div className="p-3 rounded mb-4">
                    ...
                    <p className="mb-0">
                      ...
                      {Array.isArray(allowedRoles) ? allowedRoles.map((role) => (
                        <span key={role} className="badge bg-primary ms-1">
                          {role}
                        </span>
                      )) : (
                        <span className="badge bg-info text-dark ms-1">{allowedRoles}</span>
                      )}
                    </p>
                  </div>
